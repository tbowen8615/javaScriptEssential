...
// New Function to find employees by specialization
function findBySpecialization(specialization) {
    ...
    if (specializedEmployees.length > 0) {
        const specializedDisplay = specializedEmployees.map((employee, index) => 
            `<p>${employee.id}: ${employee.name}: ${employee.name} - ${employee.department} - ${employee.specialization} - $${employee.salary}</p>`
        ).join('');
        document.getElementById('employeesDetails').innerHTML = specializedDisplay;
    } else {
        document.getElementById('employeesDetails').innerHTML = 
            `No employees found with specialization in ${specialization}.`;
    }
}
